 ...
 *
 * Handles user authentication and checks if a password reset is required, then provides the correct redirect URL.
 */
...
try {
    // --- Fetch User from Database (UPDATED to include password_reset_required) ---
    $sql = "SELECT id, password, role, first_name, status, password_reset_required FROM users WHERE staff_id = :staff_id";
    ...
    if ($user && password_verify($password, $user['password'])) {
        ...
        // --- NEW: Store the password reset flag in the session ---
        $_SESSION['password_reset_required'] = (bool)$user['password_reset_required'];

        $response['success'] = true;
        $response['message'] = 'Login successful! Redirecting...';

        // --- NEW: Determine the correct redirect URL and add it to the response ---
